"""Corpus run dirs -> the (z-signature, fork positions) training pairs of the signature map g.
...
What one pair is
    One corpus trajectory: its v3 signature (2713-d at 3B), z-scored into the
    space the discriminants were fit in, plus the continuation positions at
    which that trajectory made a choice its siblings did not. g's loss is the NLL of the
...
REFUSES a feature-name mismatch. Every gen's ``feature_names`` must equal the
    discriminants' ``FULL_names`` exactly. The extraction's Gate 0 asserts this
    at extraction time; asserting it again here is cheap and catches a run dir that
...
REFUSES to invent a divergence at continuation index 0. Position 0 has no banked
    predictive distribution (corpus stage 2 banks T = N - 1 entries; entry ``t``
    ...
    "this gen was trained on realized sibling divergences" are different data:
    where the model was unsure and where siblings actually split need not
    coincide.
...
REFUSES to leak a held-out prompt into train. ``val_prompt`` holds out WHOLE
    ...
    prompt it has seen" — and the headline contrast (matched vs SIBLING) lives
...
battery, and 384-token fork-class gens sit tens of thousands of fit-sigmas out
...
``g(own signature)`` and ``g(sibling signature)`` are identical and the headline
...
``--clip-z`` is one fix. On the 3B pilot corpus the unclipped floor IS breached
(min cosine 0.9998) and ``--clip-z 10`` restores it to ~0.90.

convergent_control gens are excluded by default. Control prompts come out
tight in the basin check (`pleroma.map.build.basin`) — their seeds do not occupy distinguishable basins, so
...
Usage (local)::
    ...
    python -m pleroma.map.build.pairs \\
        --run-dirs outputs/expB0_pilot \\
        --fork-reports outputs/expB0_pilot/fork_report.json \\
        --discriminants path/to/factor_directions_3b.npz \\
...
#: prompt_class excluded unless --include-control. Control prompts come out
#: tight, i.e. with no within-prompt basin structure to invert.
...
class PairRecord:
    """One training pair, exactly as it appears in the pairs manifest (`write_pairs`).

    ``row`` indexes the pairs matrix ``z`` — manifest order IS matrix order, and the
    ...
    ``prompt_length + p - 1`` (corpus stage 2's banked alignment, offset 1).
    """
...
@dataclass
class LoadedPairs:
    """A pairs dir's manifest + z matrix (as `write_pairs` writes them), loaded and cross-checked."""
...
def load_fork_report(path: Path) -> dict[int, dict[str, Any]]:
    """gen entries of a stage-3 fork report (`pleroma.map.build.forks`), keyed by generation_id."""
    path = path.expanduser()
    if not path.exists():
...
def load_cells(run_dir: Path) -> dict[int, dict[str, Any]]:
    """Corpus stage 2's ``<run-dir>/cells.json``, keyed by generation_id."""
    cells_path = run_dir / "cells.json"
    if not cells_path.exists():
        raise FileNotFoundError(
            f"{cells_path} not found — corpus stage 2 writes it next to signatures/"
        )
...
def load_run(
    ...
    """Load every usable gen of one corpus run dir. Returns (gens, failures).
    ...
    for gid in sorted(cells):
        ...
        try:
            ...
            if str(fork_entry.get("prompt_id", cell["prompt_id"])) != str(cell["prompt_id"]):
                raise ValueError(
                    f"fork_report prompt_id {fork_entry.get('prompt_id')!r} != cells manifest "
...
def z_scale_diagnostic(
    ...
    expository mode battery; corpus gens are 384-token continuations of *fork-class*
...
def layernorm_separability(
    ...
    Why this number and not the raw-z one: raw v3 signatures are nearly parallel
    globally, and discrimination lives in a thin Δ-layer. If the standardised
    rows are still cosine ~1.0, then ``matched`` and ``sibling`` are the same
    input to within float noise and the headline contrast is pinned at zero BY CONSTRUCTION — a null that says nothing
...
def build(
    ...
    if standardize == "corpus":
        ...
        # foreign FULL_scale leaves buried under common mode (without it the
        # inputs are near-collinear and the fit returns a null).
...
def write_pairs(
    ...
    """Write the pairs manifest, the z matrix and the run meta into ``out_dir``."""
    if not pairs:
...
def load_pairs(pairs_dir: Path) -> LoadedPairs:
    """Read back a pairs dir (`build` + `write_pairs`), re-checking manifest/matrix alignment.
    ...
    for path in (manifest_path, z_path):
        if not path.exists():
            raise FileNotFoundError(f"{path} not found — run pleroma.map.build.pairs first")
...
def main() -> int:
    ...
    parser.add_argument(
        "--run-dirs", type=Path, nargs="+", required=True, help="corpus stage-2 output dirs"
    )
    parser.add_argument(
        "--fork-reports",
        type=Path,
    ...
    parser.add_argument(
        "--include-control",
        action="store_true",
        help=f"keep {CONTROL_CLASS} gens (excluded by default; they come out tight)",
    )
    parser.add_argument(
        "--entropy-fallback-k",
        type=int,
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "clip z to +/- this many fit-sigmas (0 = off, the default). The 3B pilot "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "fix for the near-collinear inputs the FULL basis leaves; not comparable to the "
            "discriminant space)"
        ),
    )
    ...
    try:
        ...
        logger.error("pairs build failed: %s: %s", type(exc).__name__, exc)
        return 1
