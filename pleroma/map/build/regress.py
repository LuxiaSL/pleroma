"""Can the signature find the carrier? Ridge from signatures onto the cluster-contrast levers.

Regresses corpus-standardized signatures z onto the cluster-contrast levers
(`pleroma.map.build.levers`), which carry a basin onto the stream when worn.
Per gen the target is s_i * lever_g flattened over sites (s_i = +1 for cluster
A members, -1 for cluster B: the sign that points a member toward its own
basin), so cosine(prediction, target) > 0 means "z placed this trajectory on
the correct side of its prompt's basin axis."
...
prompts," which is exactly what a signature map needs and what end-to-end NLL
...
CPU + numpy only. Run where the pairs dir lives::
    ...
    python -m pleroma.map.build.regress \\
...
# Mirrors the lever build's corpus normalization for the four original run
# dirs; anything else (e.g. sharded replay dirs) falls through to the SAME
# `pleroma.map.build.hiddens.corpus_key` the hidden-means bank and the lever
# build use, so new corpora join without this table growing.
CORPUS_OF_BASENAME = {
    "pilot_replay": "pilot",
    "wave2_replay": "wave2",
...
def cv_predictions(
    ...
    The raw material for predicted levers. Every row is predicted by a model
    fitted on folds that excluded the row's whole group, so a prediction here has
    never seen its own prompt's basin geometry — which is why an evaluation of
    predicted levers can skip the cluster_sums leave-one-out and still be held
    out.
...
def emit_predictions(
    ...
    """Write the predicted-lever bank to ``path`` (`--emit-predictions`): held-out predictions, per gen.
    ...
    *i* toward ITS OWN basin — so the raw prediction ``pred_i`` points the
    ...
    the lever build stores in ``levers``. Consequences, all of them wanted:
    averaging over a group is a plain mean; ``cos(member_pred, levers[g])`` is a
    plain cosine with no sign bookkeeping; and an evaluation recovers the
    matched lever exactly as it does for a measured one —
    ``matched_sign(cluster) * lever`` — with the only difference being where the
    lever came from. ``member_signs``
    ...
    for gi in range(n_groups):
        ...
        if sel.size == 0:
            continue
        # The FULL-group average (no leave-one-out): the dilution number a
        # predicted lever is priced against. An evaluation records the LOO
        # averages it actually uses.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "this npz — the predicted-lever bank. Stores the SIGNED-CANONICAL prediction "
            ...
            "the same space as the lever build's `levers` and averaging is a plain mean"
        ),
    )
    ...
    parser.add_argument(
        ...
        help="bins feature npz, joined by (corpus, generation_id) — an extra feature block beside z",
    )
    parser.add_argument(
        "--fold-by", choices=["group", "prompt"], default="prompt",
        help="prompt (DEFAULT): both waves of a prompt share a fold. group = "
        "(prompt, wave) folds, which LEAK twin levers across folds — kept only "
        "to reproduce numbers computed that way.",
    )
    parser.add_argument(
    ...
    # Expected arrays (from `pleroma.map.build.levers`): levers [G, S, D_h]; group keys; per-gen
    ...
    # (the matched sign: toward the member's own basin).
    ...
    if args.features != "z":
        # An extra feature block (bins feature npz) beside z, joined by the
        # SAME (corpus, generation_id) key as the lever join, standardized on
        # TRAIN-split rows only — no worse a leak profile than z's own corpus
        # standardization in `pleroma.map.build.pairs`.
        from pleroma.map.build.hiddens import corpus_key as _corpus_key
    ...
    if args.fold_by == "prompt":
        # WHY FOLD BY PROMPT: folds by (prompt, wave) put a prompt's two waves
        # in DIFFERENT folds — sorted twins are adjacent, i % folds never
        # collides — so a held-out twin's sibling lever (mean |cos| .40, up to
        # .89 on some prompts) is a training target, inflating the "held-out"
        # tail. Grouping by prompt_id alone holds out BOTH waves of a prompt
        # together. `--fold-by group` is kept only to reproduce numbers
        # computed the leaky way.
