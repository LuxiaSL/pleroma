...
Implements resolution B of anamnesis' span-coverage feature class: per generated
token, attention mass into N_BINS equal relative-position bins over the PROMPT,
at a sampled set of layers, with a 3-way head summary per bin — mean over heads,
max over heads, and the entropy of the per-bin head distribution (head
*dispersion* without head identity). Pooled over generated tokens by mean. 20 x
7 x 3 = 420 dims at the defaults. Resolutions A (head-pooled) and C (per-head)
are one config change away — this module takes them as flags.

Scope note: this is pleroma's build, only as far as the map needs; it is not
part of anamnesis' standing feature suite. Features are emitted RAW with family-tagged
...
def extract_one(
    ...
    """One gen's bins-B feature vector. Separate from ``main()`` so a
    persistent process (``pleroma.harvest.worker``) can call the EXACT same
    per-gen numerics ``main()``'s loop calls, instead of a second
    implementation that could drift from this one. Raises on a bad gen (short
    prompt, zero generated tokens,
...
def load_bins_model(model_path: str, model_dtype: str, device: str) -> Any:
    ...
    Separate so ``pleroma.harvest.worker`` loads this model ONCE and reuses this
    ...
    what ``main()`` loads).
    """
    import torch  # noqa: F401 — imported before transformers, as before the move
    from transformers import AutoModelForCausalLM

    from pleroma.model.load import DTYPES

    dtype = DTYPES[str(model_dtype)]
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "order-sensitive coordinates the mean-pool discards.",
    )
    ...
    from pleroma.model.layers import decoder_layers
    from pleroma.model.load import DTYPES

    try:
        _preset = resolve_preset(args.preset)
