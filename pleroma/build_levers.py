"""Exp B1.5 stage L1b: cluster-contrast levers (local CPU, numpy + stdlib only).

PLAN-B1.5 §L1's actual object. For one prompt's seeds, split into the two basins
``basin_check`` already found, the lever is the difference of the two clusters'
...
direction, and it contains no NLL: those are the three things the B1 post-mortem
(RESULTS-B1 addendum 2) killed.

── the clusters are NOT re-invented here ──────────────────────────────────────

``two_means_split`` is IMPORTED from ``basin_check`` (basin_check.py:326) and fed
exactly what ``basin_check.analyse_prompts`` fed it: the signatures of one
...
across two runs and calling the result basin_check's.

── leave-one-out is the caller's job, and the algebra is here ─────────────────

``loo_lever()`` is exported because ``eval_levers`` must never score a gen under
...
from pleroma.basin_check import load_signatures, two_means_split
...
#: wearing a cluster's hat (the thinness worry PLAN-B1.5 §Notes names).
...
class HiddenBank:
    """Every ``mean_hiddens.npz`` given on the command line, concatenated.
...
def load_hidden_banks(paths: Sequence[Path]) -> HiddenBank:
    """Load and concatenate ``mean_hiddens.npz`` files, refusing incompatible ones."""
    required = (
...
# ── clustering: basin_check's own 2-means, on basin_check's own rows ────────────


@dataclass(frozen=True)
class ClusteredGen:
    """One gen with the cluster label ``basin_check`` gave it."""
...
def cluster_run_dir(run_dir: Path, wave_boundary: int) -> list[ClusteredGen]:
    """Re-run ``basin_check``'s per-prompt 2-means over one stage-2 run dir.

    Deliberately the same three lines ``basin_check.analyse_prompts`` runs
    (basin_check.py:386-422): prompt order is first-appearance in
    ...
    for pid in unique_prompts:
        ...
        if len(waves) != 1:
            raise ValueError(
                ...
                f"{wave_boundary}. basin_check clustered this run's seeds together, so "
    ...
    logger.info(
        "%s -> corpus %r: %d gens over %d prompts clustered (basin_check 2-means)",
...
def verify_against_basin_report(
    ...
    """Assert our labels reproduce a published ``basin_report.json``, exactly.
    ...
    for row in blob.get("prompts", []):
        ...
        if not (same or flipped):
            raise ValueError(
                ...
                "levers would not be built on basin_check's clusters."
            )
        checked += 1
    if checked == 0:
        raise ValueError(
    ...
    for key in ordered_keys:
        ...
        if len(corpora) != 1:
            raise ValueError(
                ...
                "basin_check clustered each run dir separately, so a group spanning "
...
def main() -> int:  # noqa: C901 — one linear procedure, sectioned for readability
    ...
    parser.add_argument(
        ...
        metavar="[CORPUS=]PATH",
        help=(
            "basin_check JSON(s) whose cluster assignments ours must reproduce "
    ...
    logger.info(
        "NEXT: eval_levers --levers %s --hiddens %s --gen-dirs <stage-1 dirs>",
