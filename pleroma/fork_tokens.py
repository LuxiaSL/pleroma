"""Exp B0 stage 3: where did the loomed futures actually fork? (local CPU, numpy only)

Two fork notions, deliberately kept separate and both emitted per trajectory, so
the plan's prediction is a measurement rather than a definition:
...
The prediction (plan §Predictions, P ≈ .8) is that these agree. The summary block
...
Two honesty constraints, both load-bearing:
  ...
  - Continuation index 0 has no banked predictive distribution: stage 2's replay
    ...
    (replay_extract.py:118-124). A fork there gets ``null`` entropy with the flag
    ``no_banked_distribution``.

Reads stage 2's output only (``fork_series/`` + ``cells.json``); no torch, no
anamnesis, no GPU.

Usage (local)::

    python -m pleroma.fork_tokens \\
...
@dataclass(frozen=True)
class GenCell:
    """Stage-1 metadata carried forward by stage 2's cells.json.
...
@dataclass
class GenForks:
    """Everything stage 4 / B1 needs about one trajectory's forks."""
...
def load_cells(run_dir: Path) -> dict[int, GenCell]:
    """Read stage 2's cells.json — the copied stage-1 metadata."""
...
def detect_alignment_offset(series: ForkSeries) -> int:
    ...
    Stage 2 banks ``o = 1`` (replay_extract.py:123: chosen ids are g_1..g_{N-1}).
    ...
    for i, a_id in enumerate(gen_ids):
        ...
        for b_id in gen_ids[i + 1 :]:
            ...
            if a.prompt_length != b.prompt_length:
                logger.warning(
                    "gen_%03d/gen_%03d share a prompt_id but not a prompt length "
...
def build_gen_forks(
    ...
    return GenForks(
        ...
        # plan's P ≈ .8 has to beat to mean anything.
...
def _overlap_block(gens: list[GenForks]) -> dict[str, Any]:
    ...
    return {
        ...
        # The plan's P ~ .8 prediction reads off these two lines.
...
def summarize(gens: list[GenForks]) -> dict[str, Any]:
    ...
    Class is an experimental variable in the revised plan, and the classes differ
    ...
    for prompt_id in sorted(by_prompt):
        ...
        for gid in sorted(group):
            try:
                ...
                logger.error("gen_%03d skipped: %s", gid, exc)
