"""Exp B0 stage 2: teacher-forced eager replay of the forked gens -> v3 signatures.

Stage 1 banked the realized tokens; every number B0 reports comes from here. The
...
REFUSES to run without calibration. ``--calib-dir`` must hold ``positional_means.npz``
    ...
    space; B0's whole premise is that our signatures live in the space the 3B
...
REFUSES to proceed past a feature-name mismatch (GATE 0). After the FIRST gen's
    ...
    family-config drift there is, including the pooled-vs-corrected PCA trap from
    the expA handoff. It fires on gen one so a 128-gen GPU run dies in seconds
    rather than producing a full directory of un-projectable vectors.

REFUSES to hard-code a dimension. Dim policy (Luxia, 2026-09-16): the live suite's
    dimensionality drifts per model and as families land, so every shape here
    derives at runtime from ``feature_names`` / array shapes.
...
stage 3 consumes. Stage-1 metadata is copied into ``cells.json`` so stages 3 and 4
never need ``--gen-dir``.

Adapted from anamnesis ``scripts/run_replay_extraction.py``; the feature family
flags come from ``kvrot.sigbridge.v3_family_config`` (sigbridge.py:113).

── --shard i/n and --resume (PLAN-throughput item a) ──────────────────────────
...
i/n`` takes every n-th record of the work list; ``parallel_replay_b0`` launches
...
list, a different partition, and a handful of gens that no worker owns. (Caught
in test, 2026-09-16: 4 workers over 11 gens extracted 7 of them.) The price is
...
generation_id, so their writes are disjoint. The exceptions are ``cells.json``
and ``run_meta.json``, which describe the WHOLE corpus and are therefore written
per shard under ``shard_meta/`` for the launcher to merge.

Usage (from the repo root, inside the project venv)::

    python -m pleroma.run_replay_b0 \\
        --gen-dir outputs/expB0_3b/gen --out-dir outputs/expB0_3b/replay \\
...
# Pin CPU thread pools BEFORE numpy/torch import (mirrors run_gen_tokens.py:25-31).
...
#: Where to look for kvrot/sigbridge.py: the repo root ("." — the vendored
#: kvrot/ package lives there) first, then a sibling kv-rotation checkout.
#: --kvrot-path overrides; build_family_config() has a field-for-field
#: fallback if sigbridge is unimportable everywhere.
...
def find_pca_model(calib_dir: Path) -> Path:
    ...
    ``pca_model.pkl`` is the canonical name (sigbridge.load_3b_calibration); a
...
def load_calibration_strict(calib_dir: Path) -> tuple[F32, Any, Any, dict[str, str]]:
    ...
    Mirrors ``kvrot.sigbridge.load_3b_calibration``: accepts the legacy pooled dict
    ...
    # any PCA; only tier-3 VALUES differ — the expA-handoff pooled-vs-corrected
    # trap), so the run must record exactly which bytes it used.
    provenance = {
...
def _import_sigbridge(kvrot_path: str | None):
    """Return the kvrot.sigbridge module, or None if it is not importable here."""
    ...
    try:
        from kvrot import sigbridge  # type: ignore[import-not-found]

        return sigbridge
    except Exception as exc:  # noqa: BLE001 — sigbridge pulls torch; absence is survivable
        logger.warning("kvrot.sigbridge not importable (%s); replicating its flags", exc)
        return None


def build_configs(preset_name: str, kvrot_path: str | None) -> tuple[Any, Any, str]:
    """(ExtractionConfig, FeaturePipelineConfig, provenance) for the frozen v3 space.

    Preferred: import ``v3_extraction_config`` / ``v3_family_config`` from
    kv-rotation's sigbridge, the single source of truth for the space that
    ``factor_directions_3b.npz`` was fit in. Fallback: replicate the FLAGS
    field-for-field from sigbridge.py:113. Only flags are ever replicated — every
    layer list still comes from the preset, and no dimension count appears here.
    """
    from anamnesis.config import MODEL_PRESETS, ExtractionConfig, FeaturePipelineConfig

    sigbridge = _import_sigbridge(kvrot_path)
    if sigbridge is not None:
        return (
            sigbridge.v3_extraction_config(preset_name),
            sigbridge.v3_family_config(preset_name),
            f"kvrot.sigbridge ({sigbridge.__file__})",
        )
    ...
    # Replicated field-for-field from kvrot/sigbridge.py:113 v3_family_config().
    # NOTE the deliberate omissions: value_geometry / qk_geometry / kv_cka /
    # expert_routing are NOT set here even though the current
    # run_replay_extraction.py enables them — those families post-date the
    # discriminant fit, and enabling any of them changes feature_names, which
...
# ── Fork series: the small arrays stage 3 lives on ─────────────────────────────


def fork_series_arrays(
    raw_data: Any, input_ids: list[int], prompt_length: int, top_k: int
) -> dict[str, NDArray]:
    """Extract stage 3's per-position surface from an in-memory replay pass.

    Same construction as ``raw_saver.save_raw_tensors_v3``'s logits block (top-k by
    value, exact full-vocab entropy computed before the truncation), pulled out so
    it is banked even when raw banking is off.

    Alignment (replay_extract.py:118-124): entry ``t`` is the state at continuation
    index ``t`` and its logits predict continuation index ``t+1``; ``chosen_ids[t]``
    is therefore the realized token at continuation index ``t+1``, absolute
    ``prompt_length + t + 1``. T = N - 1, so continuation index 0 has no banked
    distribution — stage 3 records that as null rather than inventing one.
    """
...
def parse_shard(spec: str) -> tuple[int, int]:
    ...
    ``parallel_replay`` passes its workers) because this stage's work list is
...
def signature_path(out_dir: Path, generation_id: int) -> Path:
    """Where ``save_features`` puts this gen's vector (feature_pipeline.py:394).

    The resume key, and the reason sharded workers can share ONE --out-dir: every
    per-gen artifact this stage writes is named by generation_id, so two shards can
    never target the same path. The whole-corpus indices (cells.json, run_meta.json)
    are the exception and are written per shard, for the launcher to merge.
    """
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "run aborts (anamnesis only warns — B0 refuses)"
        ),
    )
    parser.add_argument(
        "--discriminants",
    ...
    parser.add_argument(
        "--kvrot-path",
        default=None,
        help="kv-rotation src/ dir, for importing sigbridge's v3 config",
    )
    parser.add_argument(
        "--shard",
        default=None,
        help=(
            ...
            "workers can share one --out-dir. Applied AFTER --limit and after the "
            "resume filter, in that order, by every worker alike. Gate 0 still runs "
            "in each worker on its own first gen. Launcher: parallel_replay_b0"
        ),
    )
    parser.add_argument(
        "--resume",
    ...
    extraction_config, family_config, config_source = build_configs(
    ...
    for i, rec in enumerate(records):
        ...
        try:
            ...
            # cells.json carries stage-1 metadata forward so stages 3 and 4 never
            # need --gen-dir; the paths are relative to --out-dir.
            cells.append(
                {
    ...
    if shard_count > 1:
        ...
        # whole-corpus index: cells.json is what stages 3 and 4 read, and the last
        # worker to finish would leave its own slice there under the shared name.
        # The launcher (parallel_replay_b0) merges these into cells.json.
    ...
    if args.freeze_names_to is not None:
        ...
        # signatures. Drop-in for --discriminants everywhere (run_replay_b0
