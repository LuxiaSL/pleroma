"""Exp B1.5 stage L1a: bank each gen's MEAN hidden state at the four inject sites.

Node GPU. One teacher-forced forward per banked trajectory, reading
...
This is the raw material for the contrast levers of PLAN-B1.5 §L1: a lever is
``mean_h(cluster A) - mean_h(cluster B)`` at a site, and the only thing that has
...
that ``anamnesis.extraction.model_loader.attach_residual_write`` (a
forward_PRE_hook, model_loader.py:538-569) addresses, and exactly what
``steer_generate.attach_layers_for`` computes. The default ``8,15,19,22`` is
therefore the SAME four residual-stream sites as B1's ``--inject-layers
7,14,18,21``, which named them by the layer whose OUTPUT they are
(``g_net.TrainableResidualInjector``, a forward hook, g_net.py:178). One site,
two spellings, off by one:

    attach/site index  s   ==   inject-layer index  s - 1

and the +1 rule lives in ``steer_generate.attach_layers_for`` — never as a
literal, here or anywhere.

The third spelling is the one this stage reads:
``BaseModelOutputWithPast.hidden_states[s]``. transformers appends the residual
stream to ``all_hidden_states`` BEFORE calling decoder layer ``s``
(transformers 4.51.3 models/llama/modeling_llama.py:553-556; the same shape in
5.x), and appends the final-normed stream once more after the loop
(modeling_llama.py:586-592), so the tuple has ``n_layers + 1`` entries and
...
on the node's own transformers, before a single gen is banked. It also asserts
...
One ``mean_hiddens.npz`` per invocation (a "gen-dir group"), holding every gen
...
replay dir (``pilot_replay``) name the same corpus, and ``build_levers`` has to
...
Usage (from the repo root, inside the project venv)::
    ...
    python -m pleroma.bank_mean_hiddens \\
...
from pleroma.run_replay_b0 import load_gen_records, resolve_gen_records_dir
...
#: residual-stream sites B1 trained at under the OUTPUT-side spelling 7,14,18,21.
...
def continuation_masks(
    ...
    The position set every B1 stage means by "the continuation": training injected
    at absolute positions >= prompt_length (g_net.py:475) and ``steer_generate``
    gates on ``start_pos = prompt_length``. Padding is right-hand and excluded.
    """
...
def verify_site_indexing(
    ...
    its delta to, model_loader.py:436-450) during one probe forward that also asks
...
def main() -> int:  # noqa: C901 — one linear procedure, sectioned for readability
    ...
    parser.add_argument(
        ...
        help=(
            ...
            f"default {DEFAULT_SITES} is the same four residual-stream sites as B1's "
            "--inject-layers 7,14,18,21 (which named them by the layer whose OUTPUT "
            "they are)"
        ),
    )
    ...
    try:
        model = g_net.load_frozen_model(args.model_path, g_net.DTYPES[args.model_dtype], device)
    ...
    layers = g_net.decoder_layers(model)
    stack = g_net.decoder_stack(model)
    ...
    pad_id = g_net.resolve_pad_token_id(model, str(args.model_path))
    ...
    logger.info("NEXT: build_levers --hiddens %s --run-dirs <replay dirs>", out_npz)
    return 0
